...
class ManipulationDriver(object):
  ...
  :param state_cb: [description], defaults to None
  :type state_cb: [type], optional
  ...
  def __init__(
    ...
    self.config_path: str = rospy.get_param('~config_path', os.path.join(os.getenv('HOME', '/root'), '.ros/configs/manipulation_driver.yaml'))
  ...
  def velocity_cb(self, msg: TwistStamped) -> None:
    """
    ROS Service callback - Moves the arm at the specified cartesian velocity w.r.t. a target frame
    ...
    with self.lock:
      ...
      self.e_v = np.array([target.linear.x, target.linear.y, target.linear.z, target.angular.x, target.angular.y, target.angular.z])
  ...
  def pose_cb(self, goal: MoveToPoseGoal) -> None:
    ...
    with self.lock:
      ...
      Tep = SE3(pose.position.x, pose.position.y, pose.position.z) * UnitQuaternion([
        pose.orientation.w, 
        pose.orientation.x,
        pose.orientation.y,
        pose.orientation.z
      ...
      q_pickup = self.robot.ikine_min(Tep, q0=self.robot.qr)
      qt = rtb.tools.trajectory.jtraj(self.robot.q, q_pickup.q, 50)

      if self.__traj_move(qt):
  ...
  def servo_cb(self, goal: ServoToPoseGoal) -> None:
    """
    ROS Action Server callback - Servos the end-effector to the cartesian pose indicated by goal

    :param goal: [description]
    :type goal: ServoToPoseGoal

    This callback makes use of the roboticstoolbox p_servo function to generate velocities at each timestep.
    """    
    if self.moving:
      self.preempt()
    ...
    with self.lock:
      ...
      while not arrived and not self.preempted:
        v, arrived = rtb.p_servo(self.robot.fkine(self.robot.q), Tep, 0.5)
        self.event.clear()
  ...
  def named_pose_cb(self, goal: MoveToNamedPoseGoal) -> None:
    ...
    with self.lock:
      if not goal.pose_name in self.named_poses:
        self.named_pose_server.set_aborted(MoveToNamedPoseResult(result=1), 'Unknown named pose')
      
      qt = rtb.tools.trajectory.jtraj(self.robot.q, np.array(self.named_poses[goal.pose_name]), 50)
  ...
  def home_cb(self, req: EmptyRequest) -> EmptyResponse:
    ...
    with self.lock:
      qt = rtb.tools.trajectory.jtraj(self.robot.q, self.robot.qr, 50)
      self.__traj_move(qt)
  ...
  def get_named_pose_configs_cb(self, request: GetNamedPoseConfigsRequest) -> GetNamedPoseConfigsResponse:
  ...
  def __traj_move(self, qt: np.array) -> bool:
    self.moving = True
    for q in qt.q:
      if self.preempted:
        break
  ...
  def publish_state(self) -> None:
    ...
    pose_stamped.pose.position.x = T.t[0]
    pose_stamped.pose.position.y = T.t[1]
    pose_stamped.pose.position.z = T.t[2]

    Q = UnitQuaternion(T.R)

    pose_stamped.pose.orientation.w = Q.A[0]
    pose_stamped.pose.orientation.x = Q.A[1]
    pose_stamped.pose.orientation.y = Q.A[2]
    pose_stamped.pose.orientation.z = Q.A[3]
